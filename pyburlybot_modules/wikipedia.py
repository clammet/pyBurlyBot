...
def wiki(event: Event, bot: BotLike) -> None:
    ...
    if not event.argument:
        return bot.say(functionHelp(wiki))
    result = search(event.argument, results=1, suggestion=True)
    if not result[0]:
        if result[1]:
            return bot.say("No results found. Did you mean \x02%s\x02?" % result[1])
        else:
    ...
    try:
        ...
        while attempt < 3:
            ...
            attempt += 1
    if not p:
        return bot.say("Gave up looking for disambiguous entry from disambiguous page.")
