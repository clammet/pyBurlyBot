...
def lookup_groupalias(qfunc: DatabaseQuery, alias: str) -> str | None:
    result = qfunc(
        """SELECT grp FROM aliasgrpalias WHERE alias = ?;""", (alias,), fetchone
    )
    if not result:
        return None
    return result["grp"]
...
def add_alias(qfunc: DatabaseQuery, source: str, alias: str) -> None:
    qfunc(
        """INSERT OR REPLACE INTO alias (alias, user) VALUES (?,?);""", (alias, source)
    )


def add_groupalias(qfunc: DatabaseQuery, source: str, alias: str) -> None:
    qfunc(
        """INSERT OR REPLACE INTO aliasgrpalias (alias, grp) VALUES (?,?);""",
        (alias, source),
    )
...
def group_check(qfunc: DatabaseQuery, group: str, nick: str) -> sqlite3.Row | None:
    return qfunc(
        """SELECT 1 FROM aliasgrp WHERE grp = ? AND user = ?;""",
        (group, nick),
        fetchone,
    )
...
def unsubscripe(event: Event, bot: BotLike) -> None:
    ...
    if event.argument:
        ...
        bot.dbQuery(
            """DELETE FROM aliasgrp WHERE grp = ? AND user = ?;""", (gtarget, user)
        )
...
def listgroups(bot: BotLike, user: str | None = None) -> None:
    ...
    if user:
        result = bot.dbQuery(
            """SELECT grp from aliasgrp WHERE user = ? GROUP BY grp;""", (user,)
        )
    else:
...
def group(event: Event, bot: BotLike) -> None:
    ...
    if arg1 == "~del":
        arg1, arg2, arg3 = argumentSplit(
            event.argument, 3
        )  # rebind arguments for more params
        ...
        if arg2 and arg3:
            ...
            if group_name:
                ...
                if group_check(bot.dbQuery, group_name, nick):
                    ...
                    return bot.say(
                        "User (%s) not found in group (%s)" % (nick, group_name)
                    )
            else:
    ...
    if arg1 and arg2:
        ...
        if group_check(bot.dbQuery, group_name, nick):
            return bot.say(
                "User (%s) is already a member of (%s)." % (nick, group_name)
            )
...
def aliasgroup(bot: BotLike, groupname: str, alias: str) -> bool | None:
    ...
    if not source:
        ...
        # does group exist
        if not bot.dbQuery(
            """SELECT grp FROM aliasgrp WHERE grp = ?;""", (source,), fetchone
        ):
            return False
...
def del_alias(bot: BotLike, alias: str) -> None:
    ...
    if origin_user and origin_group:
        return bot.say(
            "Alias (%s) for user (%s) and group (%s) removed."
            % (alias, origin_user, origin_group)
        )
    elif origin_user:
    ...
    if arg1 == "~del":
        ...
        if not source:
            ...
            if aliasgroup(bot, arg1, arg2) is False:
                return bot.say(
                    "(%s) is not a group or a user I know. Try '.alias <existing_name> <new_alias>'"
                    % arg1
                )
            else:
                return
...
def init(bot: BotLike) -> bool:
    ...
    # if not exists:
    bot.dbCheckCreateTable(
        "alias_user_idx", """CREATE INDEX alias_user_idx ON alias(user);"""
    )
    bot.dbCheckCreateTable(
        "alias_group_idx", """CREATE INDEX alias_group_idx ON aliasgrp(grp,user);"""
    )
    bot.dbCheckCreateTable(
...
# mappings to methods
mappings = (
    Mapping(command="alias", function=alias),
    Mapping(command=("subscripe", "sub"), function=subscripe),
    Mapping(command="group", function=group),
    Mapping(command=("unsubscripe", "unsub"), function=unsubscripe),
)
