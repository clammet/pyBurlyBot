...
SCHEMA = Schema(
    id=NUMERIC(numtype=int, bits=64, stored=True, unique=True),
...
class IndexProcess(Process):
        ...
        with self.ix.writer() as iw:
            ...
            while True:
                try:
                    ...
                    break
                except Exception:  # noqa: BLE001 - isolate one malformed Whoosh document
                    print_exc()
    ...
    def _processSearch(
        ...
        threadident: int | None = None
        try:
            threadident, source, query, n, gb = data
            qp = self.qp.parse(query)
            results: list[SearchResult] = []
            if not SOURCE_REGEX.match(query):
                qp = qp & Term("source", source.lstrip(CHANNEL_PREFIXES).lower())
            if not gb:
                ...
                for user, count in (
                    self.searcher.search(qp, groupedby=FieldFacet(gb, maptype=Count))
                    .groups()
                    .items()
                ):
                    results.append((count, user))
        except Exception:  # noqa: BLE001 - user query/Whoosh process boundary
            self.index_p.send(
                (threadident, None)
            )  # pass None back to caller so user error can be displayed.
            print_exc()
    ...
    def run(self) -> None:
        ...
        while True:
            try:
                ...
                try:
                    if type == QUERY:
                        self._processSearch(data)
                    elif type == LOG:
                        self._processLog(data)
                    elif type == RENAME:
                        self._processRename(data)
                    else:
                        prnt("Unexpected data in logindexsearch.")
                except Exception:  # noqa: BLE001 - keep the indexing process alive per message
                    print_exc()
...
class IndexProxy(Thread):
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            try:
                type, data = self.inqueue.get(timeout=0.2)
            except Empty:
                pass
            else:
                try:
                    ...
                    if type == STOP:
                        ...
                        procpipe.send((type, data))
                except Exception:  # noqa: BLE001 - inter-process pump boundary
                    print_exc()
    ...
    def search(
        self, source: str, query: str, n: int | None, gb: str | None = None
    ) -> SearchResults:
        """Will return None if shutdown before response ready."""
        resultq: Queue[SearchResults] = Queue()
        self.inqueue.put(
            (QUERY, (resultq, current_thread().ident, source, query, n, gb))
        )
        return resultq.get()
    ...
    def logmsg(
        ...
        if message.startswith(self.cmdprefix) and not message.startswith(
            self.cmdprefix * 2
        ):
            return
...
def logsearch(event: Event, bot: BotLike) -> None:
    ...
    if iproxy:
        ...
        if not event.argument:
            return bot.say(functionHelp(logsearch))
...
def logstats(event: Event, bot: BotLike) -> None:
    ...
    if iproxy:
        ...
        bot.say(
            ...
            % (
                event.argument,
                ", ".join(("%s: %s" % (nick, count) for count, nick in results)),
            )
        )
...
def init(bot: BotLike) -> bool:
    if bot.network not in INDEX_PROXIES:
        proxy = IndexProxy(
            bot.network,
            bot.getOption("indexdir", module="logindexsearch"),
            bot.getOption("commandprefix"),
        )
        INDEX_PROXIES[bot.network] = proxy
        proxy.start()
        bot.getModule("users").REGISTER_UPDATE(bot.network, _user_rename, external=True)
    else:
...
mappings = (
    Mapping(types=["privmsged"], function=logmsg),
    Mapping(command="log", function=logsearch),
