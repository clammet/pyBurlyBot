...
try:
    ...
    raise ImportError(
        "butt.py needs Need TeX Hyphenate: http://nedbatchelder.com/code/modules/hyphenate.py"
    ) from exc
...
RE_STOPWORDS = re.compile(
    r"^\W*(" + r"|".join(STOPWORDS) + r")\W*$", flags=re.IGNORECASE
)
...
def butt(event: Event, bot: BotLike) -> None:
    ...
    if not event.argument:
        if bot.getOption("BUTTS", module="butt"):
            # get random bestbutt
            items = bot.dbQuery(
                """SELECT id, butt FROM butts ORDER BY RANDOM() LIMIT 1;""",
                func=fetchone,
            )
            if not items:
                bot.say("butt what?")
...
def butts(event: Event, bot: BotLike) -> None:
    ...
    """
    if not bot.getOption("BUTTS", module="butt"):
        return
    if not event.argument:
...
def _butt_word(word: str, butt_pass: int = 0) -> str:
    ...
    left = points[offset_index]
    length = points[offset_index + 1] - left
    # Scan left and right to consume all leading b's and trailing t's to avoid e.g.
    # !butt Bartering -> Butttering # triple t
    while (actual_word[left + length : left + length + 1]) == "t":
        length += 1
    while left > 0 and actual_word[left - 1] == "b":
        left -= 1
    sub = actual_word[left : left + length]
    butt = "butt"
    if not len(sub):
        sub = actual_word
        left = 0
    ...
    actual_word = actual_word[:left] + butt + actual_word[left + length :]
...
mappings = (
    Mapping(command="butt", function=butt),
    Mapping(command="butts", function=butts),
