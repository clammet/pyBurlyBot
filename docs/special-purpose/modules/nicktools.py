...
# Rizon-specific module; copy into pyburlybot_modules before enabling it.
...
OPTIONS = {
    "restorenick": (
        int,
        "Attempt to restore nickname every <checkevery> seconds. 0 = don't attempt, 1 = attempt,"
        " >1 forcefully recover the nick.",
        1,
    ),
...
def nickCheckAndRecover(event: Event | None = None, bot: BotLike | None = None) -> None:
    if bot is None:
        raise ValueError("nicktools timer requires a bot context")
    ...
    if bot.nickname != snick:
        if srestorenick > 1 and snickpass:
            ...
            reactor.callFromThread(
                ...
                bot.sendmsg,
                "nickserv",
                "RELEASE %s %s" % (snick, snickpass),
            )
            # delayed call for setting nick after ghost. (and hope we didn't DC in the meantime??
            reactor.callFromThread(reactor.callLater, 0.6, bot.setNick, snick)
        else:
            bot.setNick(snick)


def delayedJoin(
    joinfunc: Callable[..., None], channels: Iterable[tuple[str, ...]]
) -> None:
...
def identify(bot: BotLike, snick: str | None = None) -> bool:
    ...
    if bot.nickname == snick and passwd:
        ...
        reactor.callFromThread(reactor.callLater, 0.8, bot.notice, bot.nickname, "\x1b")
        return True
    return False
...
def nickChanged(event: Event, bot: BotLike) -> None:
    if bot.getOption("restorenick", module="nicktools"):
        ...
        if event.newname != snick:
            ...
            try:
                Timers.addtimer(
                    "NICKTOOLS_%s" % bot.network,
                    float(bot.getOption("checkevery", module="nicktools")),
                    nickCheckAndRecover,
                    reps=-1,
                    bot=bot,
                )
...
mappings = (
    Mapping(types=["preJoin"], function=preJoin),
    Mapping(types=["signedOn"], function=nickCheckAndRecover),
