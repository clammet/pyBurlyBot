...
# Example module; copy into pyburlybot_modules before enabling it.
...
OPTIONS = {
    ...
    "ignorestarting": (
        list,
        "Lines starting with items in this list will be ignored.",
        [],
    ),
}
...
def samplecommand(event: Event, bot: BotLike) -> None:
    ...
    if command == "something":
        if args:
            bot.say("%s %s" % (command, args))
        else:
            bot.say("%s" % command)
    elif command == "dothing":
        if args:
            bot.say("%s %s" % (args, command))
        else:
            bot.say("%s" % command)
    else:
...
# mappings to methods
mappings = (
    Mapping(types=["privmsged"], regex=recompile(r"^\|.*"), function=repeater),
